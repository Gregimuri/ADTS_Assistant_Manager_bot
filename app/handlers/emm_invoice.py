...
from app.handlers.flows import GROUP_TAG_HINT, answer_text, parse_name_lines, reply_emm_invoice
...
async def handle_emm_tag(
    ...
    if not names:
        if is_group_chat(message):
            await state.clear()
            await answer_text(message, GROUP_TAG_HINT.format(tag="#СчетЕММ"))
            return
